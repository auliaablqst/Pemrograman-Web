...
## 2.3 Root-Relative Path yang Baru
...
dengan versi jobsheet-05/06 yang memakai path **relatif**
(`../index.html`, `list.html`, dst. — ingat konsepnya dari
[dokumentasi jobsheet-01 §1.5](../../jobsheet-01/Dokumentasi/01-konsep-dasar.md#15-navigasi-antar-halaman-a-href)).
Sekarang semua tautan diawali garis miring (**`/`**) — disebut
**root-relative path**: "mulai dari akar (root) situs, lalu ikuti path
ini," tidak peduli halaman mana pun yang sedang membukanya.

**Kenapa perubahan ini diperlukan?** Karena `header.php` sekarang
**dipakai bersama** oleh halaman-halaman di kedalaman folder yang
**berbeda-beda** (`index.php` di root, `buku/list.php` satu folder lebih
dalam). Kalau menu ditulis dengan path relatif seperti sebelumnya, satu
tautan yang sama (misalnya ke Beranda) akan butuh `index.php` kalau
dipanggil dari root, tapi `../index.php` kalau dipanggil dari
`buku/list.php` — **tidak mungkin** menulis satu baris HTML yang benar
untuk kedua situasi itu sekaligus dalam satu file `header.php` yang
sama. Root-relative path (`/index.php`) menyelesaikan ini: alamat itu
**selalu** merujuk ke `index.php` di root situs, **tidak peduli** dari
halaman mana pun link itu diklik.

**Konsekuensi penting**, sesuai catatan di [README.md](../README.md):
karena `/` di root-relative path berarti "akar dari server", server
`php -S localhost:8000` **harus** dijalankan dari dalam folder
`jobsheet-07/` itu sendiri (bukan dari folder `kode-praktikum/` atau
`PemogramanWeb-2026/`) — supaya `/assets/css/style.css` benar-benar
mengarah ke `jobsheet-07/assets/css/style.css`, bukan ke lokasi lain
yang salah.
